...
class SpeedrunBossFields extends ConsumerStatefulWidget {
  const SpeedrunBossFields({super.key, required this.updateSubmitInfo, required this.category});
  final void Function(String key, dynamic value) updateSubmitInfo;
  final String category;

  @override
  ConsumerState<SpeedrunBossFields> createState() => _SpeedrunDomainState();
}

class _SpeedrunDomainState extends ConsumerState<SpeedrunBossFields> {
  bool _initRegion = false;
  final TextEditingController _gameVersionController = TextEditingController();
  final TextEditingController _videoLinkController = TextEditingController();
  final _bossKey = GlobalKey<DropdownSearchState<String>>();
    ...
    return Column(mainAxisAlignment: MainAxisAlignment.start, children: [
      if (MediaQuery.of(context).size.width > 600) ...[
        Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          SizedBox(width: 10.wr),
          Expanded(flex: 1, child: bossField(context)),
        ]),
      ] else ...[
        regionField(context),
  ...
  Widget bossField(BuildContext context) {
    return GestureDetector(
        onVerticalDragUpdate: (details) {
          _dragAccumulator += details.primaryDelta!;
          const double dragThreshold = 10.0;
          ...
          final currentValue = _bossKey.currentState?.getSelectedItem;

          if (currentValue == null) {
            _bossKey.currentState?.changeSelectedItem(items.first);
            return;
          }
          ...
          if (_dragAccumulator > dragThreshold) {
            if (currentIndex < items.length - 1) {
              _bossKey.currentState?.changeSelectedItem(items[currentIndex + 1]);
            }
            _dragAccumulator = 0.0;
          } else if (_dragAccumulator < -dragThreshold) {
            if (currentIndex > 0) {
        ...
        child: SizedBox(
          ...
          child: DropdownSearch<String>(
              key: _bossKey,
              items: (f, cs) => widget.category == 'Weekly Boss' ? WEEKLY_BOSSES : WORLD_BOSSES,
              popupProps: PopupProps.menu(
                  fit: FlexFit.loose,
                  menuProps: MenuProps(
                    backgroundColor: Theme.of(context).colorScheme.surfaceContainerHighest,
              ...
              autoValidateMode: AutovalidateMode.onUserInteraction,
              selectedItem: widget.category == 'Weekly Boss' ? WEEKLY_BOSSES[0] : WORLD_BOSSES[0],
              decoratorProps: DropDownDecoratorProps(
                  baseStyle: const TextStyle(fontSize: 16),
                  decoration: InputDecoration(
                      labelText: 'Enemy',
